...
def SSD_MobileNet(num_classes,feature_layers):
  ...
  for index, output in enumerate(outputs):
    assert output.shape[1:3] == feature_layers[index]['shape'], 'Shapes do not match. Model output_shape is %s, expected shape is %s.' % (output.shape[1:3], feature_layers[index]['shape'])
    num_df = len(feature_layers[index]['aspect_ratios']) + 1

    # class
    cls = Conv2D(filters = num_df * num_classes, kernel_size = (1,1), padding = 'same', kernel_initializer = 'glorot_normal', name = 'cls_predictor_%s' %index)(output)
    cls = Reshape([-1, num_classes], name = 'reshape_cls_%s' %index)(cls)
    cls_total.append(cls)

    # localisation
    loc = Conv2D(filters = num_df * 4, kernel_size = (1,1), padding = 'same', kernel_initializer = 'glorot_normal', name = 'loc_predictor_%s' %index)(output)
